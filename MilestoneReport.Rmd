...
#options(java.home="C:\\Program Files\\Java\\jre1.8.0_281\\")
#library(rJava)
library(RWeka)
library(ggplot2)
```
...
```{r}
tdm  <- TermDocumentMatrix(sample_data, control = list(tokenize = trigramTokenizer))
m <- as.matrix(tdm)
dev.new(width = 1000, height = 1000, unit = "px")
word.freq <- sort(rowSums(m), decreasing = T)
library(wordcloud)
wordcloud(words = names(word.freq),  max.words =100, scale=c(2,.5), freq = word.freq, min.freq =40, random.order = F, colors = brewer.pal(8, "Blues"))
```
...
Reference: https://github.com/sreenivasulu-parimi/Data-Science-Capstone-Milestone-Report